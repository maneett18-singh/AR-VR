...
Clone: git clone -b dev https://github.com/kashif003/AR-VR.git.
Open in Unity Hub.
Import Pico Unity SDK (Assets > Import Package).
Configure XR: Install OpenXR, enable Pico/SteamVR.
Verify IPs: Set server IP in WSManager and FullNetwork/NetworkManager.
Connect Pico via USB/WiFi.
2. Network Configuration
Both server PC and Pico on same hotspot.
Note server IP (e.g., ifconfig or ip addr).
3. Start Server
cd to repo root (where server.py is).
Run: python3 server.py (install deps if needed, e.g., pip install websockets).
Confirm server logs show listening IP/port.
4. Build & Run Client
...
Pico Neo
Android
Set IP, build APK, sideload, run after server start.
SteamVR
PC Standalone
Build exe, run with SteamVR & server on.
Project Structure
text
AR-VR/
├── Assets/
