                ...
                try:
                    ...
                    if len(act.shape) == 3:  # Conv output: [C, H, W]
                        ...
                        for ch in range(act.shape[0]):
                            ...
                            # 🔽 Downsample to smaller H', W' to reduce message size
                            act_2d_small = F.interpolate(
                                act_2d.unsqueeze(0).unsqueeze(0),  # [1,1,H,W]
                                size=(14, 14),                     # adjust if needed
                                mode="bilinear",
                                align_corners=False
                        ...
                        continue  # Skip rest for conv layers

                    elif len(act.shape) == 1:  # FC output: keep as-is
                    ...
                    # Normalize to 0-255 (FC layers only)
                    act_np = act_2d.cpu().numpy()
                    act_min, act_max = act_np.min(), act_np.max()
                    if act_max > act_min:
                        act_normalized = (
                            (act_np - act_min) / (act_max - act_min) * 255
                        ).astype(np.uint8)
                    else:
                        act_normalized = (act_np * 255).astype(np.uint8)
                    ...
                    # Extract layer type from name (e.g., "fc1" from "fc1.weight")
                    layer_key = (
                        layer_name.split(".")[0]
                        if "." in layer_name
                        else layer_name
